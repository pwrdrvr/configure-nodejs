...
def suggest_version(last_release: ReleaseInfo | None, meaningful_commits: list[dict[str, Any]]) -> dict[str, Any]:
    if not meaningful_commits:
        return {
            "tag": None,
            "kind": "none",
            "reason": "No meaningful commits were found after filtering release housekeeping; do not cut a release unless the user explicitly overrides.",
...
def render_markdown(plan: dict[str, Any]) -> str:
    ...
    lines.append(f"- Commits on `{plan['repository']['defaultBranch']}` since last release: `{plan['counts']['rawCommitCount']}`")
    lines.append(f"- Meaningful commits after filtering housekeeping: `{plan['counts']['meaningfulCommitCount']}`")
    suggested_tag = plan["suggestedVersion"]["tag"] or "none"
    lines.append(f"- Suggested tag: `{suggested_tag}` ({plan['suggestedVersion']['kind']})")
    ...
    lines.append("")
    if plan["suggestedVersion"]["tag"]:
        lines.append(f"Use this heading in `CHANGELOG.md`: `## {plan['suggestedVersion']['tag']} - {plan['generatedAtDate']}`")
    else:
        lines.append("No changelog heading is suggested because there are no meaningful commits to release.")
    lines.append("")
...
def main() -> int:
    ...
    print(f"Suggested tag: {suggested_version['tag'] or 'none'}")
